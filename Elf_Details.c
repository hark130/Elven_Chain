...
void print_elf_details(struct Elf_Details* elven_file, unsigned int sectionsToPrint, FILE* stream)
{
    ...
    if ((sectionsToPrint & PRINT_ELF_HEADER) || (sectionsToPrint & PRINT_EVERYTHING))
    {
        ...
        if (elven_file->processorType == ELF_H_CLASS_32 || elven_file->processorType == ELF_H_CLASS_64)
        {
            ...
            print_binary(stream, &(elven_file->flags), sizeof(elven_file->flags), elven_file->bigEndian);
            fprintf(stream, "\n");
        }
        // ??-bit Processor
        else
...
int parse_program_header(struct Prgrm_Hdr_Details* program_struct, char* program_contents, struct Elf_Details* elven_file)
{
    ...
    else if (program_struct->processorType == ELF_H_CLASS_64)
    {
        ...
        {
            // fprintf(stdout, "Num program header entries is:\t%d\n", program_struct->prgmHdrEntrNum);  // DEBUGGING
            for (i = 0; i < program_struct->prgmHdrEntrNum; i++)
            {
                // fprintf(stdout, "Parsing program header segment:\t%d\n", i + 1);  // DEBUGGING
                ...
                if (segment64_ptr)
                {
                    ...
                    // 2.10.A.3. uint64_t segOffset;  // Offset of the segment's first byte in the file image
                    // fprintf(stdout, "segment64_ptr->segOffset is of size %d\n",sizeof(segment64_ptr->segOffset));  // DEBUGGING
                    ...
                    // 2.10.A.4. uint64_t segVirtualAddr;  // Virtual address of the segment in memory
                    ...
                    // 2.10.A.5. uint64_t segPhysicalAddr;  // Physical address of the segment in memory
                    ...
                    // 2.10.A.7. uint64_t segMemSize;  // Size in bytes of the segment in memory
                    ...
                    // 2.10.A.9. uint64_t alignment;  // Alignment
                ...
                // fprintf(stdout, "Completed parsing program header segment:\t%d\n", i + 1);  // DEBUGGING
            }
        }
        else
        {
            ...
            if (segmentArray32_ptr)
            {
                for (segmentNum = 0; segmentNum < program_struct->prgmHdrEntrNum; segmentNum++)
                {
                    ...
                    if (segment32_ptr)
                    {
                        ...
                        fprintf(stream, "\tFile Size:\t0x%" PRIx32 " (%" PRIu32 ")\n", segment32_ptr->segFileSize, segment32_ptr->segFileSize);
                        fprintf(stream, "\tMem Size:\t0x%" PRIx32 " (%" PRIu32 ")\n", segment32_ptr->segMemSize, segment32_ptr->segMemSize);
                        /* PRINT THE FLAGS */
                        fprintf(stream, "\tFlags:\t\t");
                        // Binary printer
                        // Printing flags so endianness shouldn't matter
            ...
            if (segmentArray64_ptr)
            {
                for (segmentNum = 0; segmentNum < program_struct->prgmHdrEntrNum; segmentNum++)
                {
                    ...
                    if (segment64_ptr)
                    {
                        ...
                        {
                            // fprintf(stream, "FLAGS:\t%" PRIu64 "\n", segment64_ptr->flags);  // DEBUGGING
                            // fprintf(stream, "READ:\t%d\n", segment64_ptr->flags & ELF_H_64_FLAG_R);  // DEBUGGING
                            // fprintf(stream, "WRITE:\t%d\n", segment64_ptr->flags & ELF_H_64_FLAG_W);  // DEBUGGING
                            // fprintf(stream, "EXEC:\t%d\n", segment64_ptr->flags & ELF_H_64_FLAG_X);  // DEBUGGING
                            fprintf(stream, "\t\t\t");
                            // Read
                            ...
                            // Execute
                            if ((segment64_ptr->flags & ELF_H_64_FLAG_X) == ELF_H_64_FLAG_X)
                            {
                                fprintf(stream, "Execute ");
                            }
                            fprintf(stream, "\n");
                        ...
                        fprintf(stream, "\tFile Size:\t0x%" PRIx64 " (%" PRIu64 ")\n", segment64_ptr->segFileSize, segment64_ptr->segFileSize);
                        fprintf(stream, "\tMem Size:\t0x%" PRIx64 " (%" PRIu64 ")\n", segment64_ptr->segMemSize, segment64_ptr->segMemSize);
